...
class OraDB:
    INFO = 'scott/tiger@192.168.19.9:1522/orcl'
    ADMIN = None
...
class ImageToCSV:
    __IMAGE_PATH = 'C:\python\source\crwal_cp\img2\\'
    __RESIZED_IMAGE_PATH = __IMAGE_PATH + 'resized\\'
    __LABEL_PATH = 'C:\python\source\crwal_cp\\xray_labels1.csv'
    ...
    def _setLabel(self):
        file = open('C:\python\source\crwal_cp\\xray_labels1.csv', 'r')
...
con = cx_Oracle.connect('scott/tiger@192.168.19.9:1522/orcl') #접속
cursor = con.cursor()
emp_table = cursor.execute('select * from py_test')  #이터레이터한 메모리형ㄺ태의 조회결과
